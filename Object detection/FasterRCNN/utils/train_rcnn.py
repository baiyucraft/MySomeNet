...
def train_epoch(net, train_iter, updater, device, mode='train'):
    # RPN 中的类别和框的损失，主要针对的是 目标和背景
    loss_objectness = 0
    loss_rpn_box_reg = 0
    # ROI 中的类别和框的损失，基于RPN后 进行特征提取池化 再针对 具体目标
    loss_classifier = 0
    loss_roi_box_reg = 0
    count = 0
    for X, y in tqdm(train_iter):
        if mode == 'train':
            ...
            net.eval()
        losses = net(X, y, device)
        # print(losses)

        all_loss = sum([los_value for los_value in losses.values()])
        ...
        loss_roi_box_reg += losses['loss_box_reg'].item()
        count += 1

    return loss_objectness / count, loss_rpn_box_reg / count, loss_classifier / count, loss_roi_box_reg / count
...
def train(net, train_iter, num_epoch, updater, updater_scheduler, device, net_param_path, log_path,
    ...
    for epoch in range(num_epoch):
        # train
        loss_objectness, loss_rpn_box_reg, loss_classifier, loss_roi_box_reg = \
            train_epoch(net, train_iter, updater, device, mode='train')
        print(f'{epoch + 1}, '
              f'{loss_objectness:.3f}, {loss_rpn_box_reg:.3f}, {loss_classifier:.3f}, {loss_roi_box_reg:.3f}')
        updater_scheduler.step(loss_objectness + loss_rpn_box_reg + loss_classifier + loss_roi_box_reg)
        ...
        if train_threshold is None:
            ...
            update_log(log_path, [loss_objectness, loss_rpn_box_reg, loss_classifier, loss_roi_box_reg])
            print('save model param and log')
            train_threshold = loss_classifier
        elif loss_classifier < train_threshold:
            torch.save(net.state_dict(), net_param_path)
            update_log(log_path, [loss_objectness, loss_rpn_box_reg, loss_classifier, loss_roi_box_reg])
            print('save model param and log')
            train_threshold = loss_classifier
...
def plot_log():
    with open(f'../logs/log.txt', 'r', encoding='utf-8') as f:
