...
class LoadImagesAndLabels(Dataset):
    ...
    cache_version = 0.5  # datasets labels *.cache version
...
def dataset_stats(path='coco128.yaml', autodownload=False, verbose=False, profile=False, hub=False):
    ...
    def unzip(path):
        ...
        if str(path).endswith('.zip'):  # path is data.zip
            ...
            dir = path.with_suffix('')  # datasets directory == zip name
            return True, str(dir), next(dir.rglob('*.yaml'))  # zipped, data_dir, yaml_path
        else:  # path is data.yaml
            return False, None, path

    def hub_ops(f, max_dim=1920):
        # HUB ops for 1 image 'f': resize and save at reduced quality in /datasets-hub for web/app viewing
        f_new = im_dir / Path(f).name  # datasets-hub image filename
        try:  # use PIL
            im = Image.open(f)
